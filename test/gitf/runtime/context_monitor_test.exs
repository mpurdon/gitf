defmodule GiTF.Runtime.ContextMonitorTest do
  ...
  setup do
    ...
    store_dir =
      Path.join(System.tmp_dir!(), "gitf_context_test_#{:erlang.unique_integer([:positive])}")
  ...
  describe "record_usage/3" do
    test "records token usage and calculates percentage", %{ghost_id: ghost_id} do
      ...
      assert {:ok, :normal} = ContextMonitor.record_usage(ghost_id, 40_000, 20_000)
    ...
    test "returns warning status at 40% threshold", %{ghost_id: ghost_id} do
      ...
      assert {:ok, :warning} = ContextMonitor.record_usage(ghost_id, 80_000, 40_000)
    ...
    test "returns critical status at 45% threshold", %{ghost_id: ghost_id} do
      ...
      assert {:ok, :critical} = ContextMonitor.record_usage(ghost_id, 90_000, 45_000)
    ...
    test "returns transfer_needed at 50% threshold", %{ghost_id: ghost_id} do
      ...
      assert {:ok, :transfer_needed} = ContextMonitor.record_usage(ghost_id, 100_000, 50_000)
    ...
    test "accumulates token usage across multiple calls", %{ghost_id: ghost_id} do
      assert {:ok, :normal} = ContextMonitor.record_usage(ghost_id, 20_000, 10_000)
      assert {:ok, :normal} = ContextMonitor.record_usage(ghost_id, 40_000, 10_000)
      assert {:ok, :warning} = ContextMonitor.record_usage(ghost_id, 80_000, 20_000)
  ...
  describe "get_usage_percentage/1" do
    test "returns current percentage", %{ghost_id: ghost_id} do
      ContextMonitor.record_usage(ghost_id, 60_000, 30_000)
  ...
  describe "get_usage_stats/1" do
    test "returns complete usage statistics", %{ghost_id: ghost_id} do
      ContextMonitor.record_usage(ghost_id, 80_000, 40_000)
  ...
  describe "get_latest_snapshot/1" do
    test "returns most recent snapshot", %{ghost_id: ghost_id} do
      ...
      ContextMonitor.record_usage(ghost_id, 80_000, 20_000)
