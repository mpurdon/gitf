defmodule GiTF.Runtime.CopilotTest do
  ...
  describe "spawn_headless/3" do
    test "returns error for invalid working directory" do
      result =
        Copilot.spawn_headless("/nonexistent/dir/#{:erlang.unique_integer([:positive])}", "hello")

      assert result in [{:error, :invalid_working_dir}, {:error, :not_found}]
    end

    test "returns error when copilot is not found" do
      original_path = System.get_env("PATH")
      System.put_env("PATH", "/empty")

      result = Copilot.spawn_headless(@tmp_dir, "hello")
  ...
  describe "spawn_interactive/2" do
    test "returns error for invalid working directory" do
      result =
        Copilot.spawn_interactive("/nonexistent/dir/#{:erlang.unique_integer([:positive])}")
