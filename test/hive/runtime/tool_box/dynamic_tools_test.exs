defmodule Hive.Runtime.ToolBox.DynamicToolsTest do
  use ExUnit.Case, async: false

  alias Hive.Runtime.ToolBox.DynamicTools
  alias Hive.Store

  setup do
    store_dir = Path.join(System.tmp_dir!(), "hive-dyntools-test-#{:rand.uniform(100_000)}")
  ...
  describe "tool provider integration" do
    test "picks up registered tool providers" do
      ...
      Hive.Plugin.Registry.register(:tool_provider, "mock", mock_module)
      ...
      # Clean up
      Hive.Plugin.Registry.unregister(:tool_provider, "mock")
    end
  end
  ...
  defp define_mock_provider do
    ...
    unless Code.ensure_loaded?(module_name) do
      Module.create(
        module_name,
        quote do
          @behaviour Hive.Plugin.ToolProvider
