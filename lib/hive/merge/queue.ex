defmodule Hive.Merge.Queue do
  ...
  a time, merged in optimal order (via `Hive.Merge.Strategy`), and the
  ...
  Registered in `Hive.Registry` under `:merge_queue`.
  At most one MergeQueue process per hive.
  """
  ...
  @impl true
  def init(_opts) do
    Phoenix.PubSub.subscribe(Hive.PubSub, "merge:queue")
  ...
  def handle_info({:DOWN, ref, :process, _pid, reason}, %{active: {job_id, _, ref}} = state) do
    ...
    Hive.Waggle.send("merge_queue", "queen", "merge_failed",
      "Merge task crashed for job #{job_id}: #{inspect(reason)}")

    Hive.Telemetry.emit([:hive, :merge, :crashed], %{}, %{
      job_id: job_id,
  ...
  def handle_info({:merge_timeout, ref, task_pid}, %{active: {job_id, _, ref}} = state) do
    ...
    Hive.Telemetry.emit([:hive, :merge, :timeout], %{}, %{
      job_id: job_id,
  ...
  def handle_info(:recover_pending, state) do
    ...
    recovered =
      ...
      |> Enum.flat_map(fn job ->
        case Hive.Store.find_one(:cells, fn c ->
  ...
  defp maybe_process_next(%{active: nil, pending: pending} = state) do
    ordered = Hive.Merge.Strategy.optimal_order(pending)
    ...
    case ordered do
      [{job_id, cell_id} | rest] ->
        ...
        task = Task.async(fn ->
          result = Hive.Merge.Resolver.resolve(job_id, cell_id)
  ...
  defp handle_merge_result(job_id, {:ok, :merged, tier}, state) do
    ...
    case Hive.Store.get(:jobs, job_id) do
      nil -> :ok
      job -> Hive.Store.put(:jobs, Map.put(job, :merged_at, DateTime.utc_now()))
    end

    # Waggle Queen
    Hive.Waggle.send("merge_queue", "queen", "job_merged",
