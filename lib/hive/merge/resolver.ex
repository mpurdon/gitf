defmodule Hive.Merge.Resolver do
  @moduledoc """
  ...
  defp attempt_tiers(job_id, _cell_id, _cell, _comb, _target, tier) when tier > 3 do
    ...
    Hive.Telemetry.emit([:hive, :merge, :exhausted], %{}, %{
      job_id: job_id,
      tiers_attempted: 4
    })
    ...
    if tier in [1, 2] and History.should_skip_tier?(tier, changed) do
      ...
      case result do
        ...
        {:error, reason} ->
          ...
          Hive.Telemetry.emit([:hive, :merge, :tier_failed], %{}, %{
            job_id: job_id,
            tier: tier,
  ...
  defp run_tier(0, job_id, cell, comb, target) do
    ...
    with_merge_lock(comb.id, fn ->
      ...
      with :ok <- Hive.Git.checkout(repo, target),
           :ok <- Hive.Git.merge(repo, cell.branch, no_ff: true) do
  ...
  defp run_tier(1, job_id, cell, comb, target) do
    ...
    with_merge_lock(comb.id, fn ->
      ...
      with :ok <- Hive.Git.checkout(repo, target) do
        # Attempt merge, expecting conflicts
        case Hive.Git.safe_cmd( ["merge", "--no-commit", "--no-ff", cell.branch],
      ...
      **Your task:** Reimplement the intent of the original changes on top of the
      ...
      case Hive.Jobs.create(attrs) do
        {:ok, reimagine_job} ->
          ...
          Hive.Waggle.send("merge_resolver", "queen", "reimagine_job_created",
    ...
    Enum.count(conflicted, fn file ->
      cond do
        additive_file?(file) ->
          ...
          case Hive.Git.safe_cmd( ["checkout", "--union", "--", file],
                 cd: repo, stderr_to_stdout: true) do
            {_, 0} ->
              Hive.Git.safe_cmd( ["add", file], cd: repo, stderr_to_stdout: true)
              true
        ...
        file_only_touched_by_branch?(repo, file, cell.branch, target) ->
          ...
          case Hive.Git.safe_cmd( ["checkout", "--theirs", "--", file],
                 cd: repo, stderr_to_stdout: true) do
            {_, 0} ->
              Hive.Git.safe_cmd( ["add", file], cd: repo, stderr_to_stdout: true)
              true
  ...
  defp file_only_touched_by_branch?(repo, file, branch, target) do
    ...
    case Hive.Git.safe_cmd( ["merge-base", branch, target],
           cd: repo, stderr_to_stdout: true) do
      {base, 0} ->
        base = String.trim(base)

        case Hive.Git.safe_cmd( ["diff", "--name-only", "#{base}..#{target}", "--", file],
               cd: repo, stderr_to_stdout: true) do
    ...
    case File.read(file_path) do
      {:ok, content} ->
        if String.contains?(content, "<<<<<<<") do
          ...
          case Hive.Runtime.Models.generate_text(prompt, model: "haiku", max_tokens: 8192) do
  ...
  defp get_head(repo) do
    case Hive.Git.safe_cmd( ["rev-parse", "HEAD"], cd: repo, stderr_to_stdout: true) do
      {output, 0} -> String.trim(output)
      _ -> nil
    end
  end
  ...
  defp rollback(repo, head) do
    abort_merge(repo)
    Hive.Git.safe_cmd( ["reset", "--hard", head], cd: repo, stderr_to_stdout: true)
    :ok
  end

  defp abort_merge(repo) do
    Hive.Git.safe_cmd( ["merge", "--abort"], cd: repo, stderr_to_stdout: true)
    :ok
  end

  defp commit_merge(repo, branch, target) do
    Hive.Git.safe_cmd( ["commit", "--no-edit", "-m", "Merge #{branch} into #{target}"],
      cd: repo, stderr_to_stdout: true)
    :ok
  end

  defp get_conflicted_files(repo) do
    case Hive.Git.safe_cmd( ["diff", "--name-only", "--diff-filter=U"],
           cd: repo, stderr_to_stdout: true) do
  ...
  defp get_changed_files(repo, branch, target) do
    case Hive.Git.safe_cmd( ["diff", "--name-only", "#{target}...#{branch}"],
           cd: repo, stderr_to_stdout: true) do
      {output, 0} -> String.split(output, "\n", trim: true)
      _ -> []
    end
  ...
  defp get_branch_diff(repo, branch, target) do
    ...
    summary =
      case Hive.Git.safe_cmd( ["diff", "--stat", "#{target}...#{branch}"],
             cd: repo, stderr_to_stdout: true) do
