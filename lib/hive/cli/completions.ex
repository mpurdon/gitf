defmodule Hive.CLI.Completions do
  @moduledoc """
  Generates shell completion scripts for bash, zsh, and fish.

  Each generator builds a string from the command tree defined in this
  module. The command tree mirrors the dispatch structure in `Hive.CLI`.
  """
  ...
  @command_tree %{
    ...
    "jobs" => ~w(list show create reset),
    "costs" => ~w(summary record),
    "waggle" => ~w(list show send),
    "cell" => ~w(list clean),
  ...
  defp bash_completions do
    ...
    """
    # Bash completions for hive
    # Add to ~/.bashrc: eval "$(hive completions bash)"
  ...
  defp zsh_completions do
    ...
    """
    # Zsh completions for hive
    # Add to ~/.zshrc: eval "$(hive completions zsh)"
  ...
  defp fish_completions do
    top_lines =
      @top_commands
      |> Enum.map(fn cmd ->
        "complete -c hive -n '__fish_use_subcommand' -a '#{cmd}'"
      end)
      |> Enum.join("\n")
    ...
    sub_lines =
      ...
      |> Enum.flat_map(fn {cmd, subs} ->
        Enum.map(subs, fn sub ->
          "complete -c hive -n '__fish_seen_subcommand_from #{cmd}' -a '#{sub}'"
        end)
      end)
      |> Enum.join("\n")
    ...
    # Add to ~/.config/fish/completions/hive.fish
