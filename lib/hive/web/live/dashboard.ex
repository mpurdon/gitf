defmodule Hive.Web.Live.Dashboard do
  ...
  def mount(_params, _session, socket) do
    ...
    socket =
      ...
      |> assign(:events, [])
      |> assign(:health, safe_call(fn -> Hive.Observability.Health.check() end, %{status: :unknown, checks: %{}, timestamp: DateTime.utc_now()}))
      |> assign(:alerts, safe_call(fn -> Hive.Observability.Alerts.check_alerts() end, []))
      |> assign(:merge_queue, safe_call(fn -> Hive.Merge.Queue.status() end, %{pending: [], active: nil, completed: []}))
      |> assign(:runs, safe_call(fn -> Hive.Run.list(status: "active") end, []))
      |> assign(:event_store_events, [])
      |> assign(:event_types, safe_call(fn -> Hive.EventStore.event_types() end, []))
      |> assign(:agent_identities, safe_call(fn -> Hive.AgentIdentity.list() end, []))
      |> assign(:checkpoints, %{})
  ...
  def handle_event("filter_events", %{"type" => ""}, socket) do
    socket =
      socket
      |> assign(:event_type_filter, nil)
      |> assign(:event_store_events, safe_call(fn -> Hive.EventStore.list(limit: 50) end, []))
  ...
  def handle_event("filter_events", %{"type" => type}, socket) do
    ...
    socket =
      ...
      |> assign(:event_store_events, safe_call(fn -> Hive.EventStore.list(limit: 50, type: type_atom) end, []))
  ...
  def handle_event("view_quest_timeline", %{"id" => quest_id}, socket) do
    events = safe_call(fn -> Hive.EventStore.list(limit: 50, quest_id: quest_id) end, [])
  ...
  defp refresh_core(socket) do
    socket
    |> assign_stats()
    |> assign(:merge_queue, safe_call(fn -> Hive.Merge.Queue.status() end, socket.assigns.merge_queue))
    |> assign(:runs, safe_call(fn -> Hive.Run.list(status: "active") end, socket.assigns.runs))
    |> assign(:alerts, safe_call(fn -> Hive.Observability.Alerts.check_alerts() end, socket.assigns.alerts))
  end
  ...
  defp maybe_refresh_slow(socket, count) when rem(count, 5) == 0 do
    ...
    socket
    |> assign(:health, safe_call(fn -> Hive.Observability.Health.check() end, socket.assigns.health))
    |> assign(:agent_identities, safe_call(fn -> Hive.AgentIdentity.list() end, socket.assigns.agent_identities))
  ...
  defp maybe_refresh_tab(%{assigns: %{active_tab: :events}} = socket) do
    ...
    assign(socket, :event_store_events, safe_call(fn -> Hive.EventStore.list(opts) end, socket.assigns.event_store_events))
  end
  ...
  defp load_budget_status(quests) do
    ...
    |> Enum.map(fn q ->
      budget = Hive.Budget.budget_for(q[:id])
  ...
  def render(assigns) do
    ...
    <div class="min-h-screen p-6 flex flex-col">
      ...
      <div class="text-center text-gray-500 text-xs py-2">Hive v<%= Application.spec(:hive, :vsn) |> to_string() %></div>
  ...
  defp render_header(assigns) do
    ...
    <header class="mb-6 flex justify-between items-center border-b border-gray-700 pb-4">
      <div class="flex items-center gap-4">
        <div>
          <h1 class="text-3xl font-bold text-yellow-500">Hive Factory Floor</h1>
