defmodule Hive.ViewModel do
  ...
  `"hive:view_model"` on every state change.
  ...
  ## Topics consumed
      ...
      "hive:progress"   — bee progress updates
      "hive:system"     — system events
      "hive:intent"     — intent events (for tracking)

  ## Topic published

      "hive:view_model" — `{:view_model, snapshot}` on each state change
  """
  ...
  defp safe_list(collection) do
    Hive.Store.all(collection)
  rescue
    _ -> []
  end

  defp safe_costs do
    costs = Hive.Store.all(:costs)
