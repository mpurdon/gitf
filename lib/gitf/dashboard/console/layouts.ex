defmodule GiTF.Dashboard.Console.Layouts do
  ...
  **Components, not repeated markup.** A ministry's identity block was written
  ...
  the primitives live in `GiTF.Dashboard.Surface.Components` and this file only
  styles them.
  """
  ...
  # HEEx treats the contents of <style> as raw text — an interpolation in there
  # is emitted literally — so the sheet is assembled in Elixir and injected as
  # one blob. The shared half is `Surface.base/0`; what follows is only what is
  # particular to the Console: its rail, its tree, its facets, its rule editor.
  @own """
  /* -------- frame -------- */
