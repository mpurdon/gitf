defmodule GiTF.Dashboard.StudioLive do
  ...
  def render(%{session_id: nil} = assigns) do
    ...
    <.live_component module={GiTF.Dashboard.AppLayout} id="layout" current_path={@current_path} flash={@flash} toasts={@toasts}>
      ...
      <% else %>
        <p style="color:var(--ink-3)">Starting session…</p>
      <% end %>
    </.live_component>
    """
  end
    ...
    <.live_component module={GiTF.Dashboard.AppLayout} id="layout" current_path={@current_path} flash={@flash} toasts={@toasts}>
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.75rem">
        ...
        <div style="display:flex; gap:0.35rem; align-items:center">
          ...
          <% end %>
          <span style="font-size:0.7rem; color:var(--ink-3); margin-left:0.4rem"
        ...
        <div class="card" style="display:flex; flex-direction:column; height:calc(100vh - 180px)">
          <div id="studio-transcript" style="flex:1; overflow-y:auto; display:flex; flex-direction:column; gap:0.6rem; padding-right:0.25rem" phx-hook="ScrollBottom">
            <%= for entry <- @studio.transcript do %>
              <div style={transcript_style(entry.role)}>
                <div style="font-size:0.7rem; color:var(--ink-3); margin-bottom:0.15rem">{entry.role}</div>
                <div style="white-space:pre-wrap; font-size:0.85rem">{entry.text}</div>
              </div>
            <% end %>
            <%= if @studio.status == :thinking do %>
              <div style="color:var(--ink-3); font-size:0.8rem">planner is thinking…</div>
            <% end %>
          </div>
          <form phx-submit="send" style="display:flex; gap:0.5rem; margin-top:0.6rem">
            <input type="text" name="message[text]" value={@message} placeholder="Talk to the planner…" autocomplete="off"
                   style="flex:1; background:var(--ground); border:1px solid var(--line); border-radius:6px; color:var(--ink); padding:0.5rem 0.7rem" />
                    ...
                    <div style="display:flex; gap:0.6rem; flex-wrap:wrap">
                      <%= for scheme <- prp.args["schemes"] || [] do %>
                        <div style="flex:1; min-width:200px; border:1px solid var(--line); border-radius:6px; padding:0.5rem; background:var(--ground)">
                          ...
                          <div style="font-size:0.78rem; color:var(--ink); margin-bottom:0.3rem">{scheme["thesis"]}</div>
      ...
      <%= if @show_approve do %>
        <div style="position:fixed; inset:0; background:rgba(0,0,0,0.6); display:flex; align-items:center; justify-content:center; z-index:50">
          <div class="card" style="width:440px">
            ...
            <p style="color:var(--ink-3); font-size:0.85rem">{length(@studio.roadmap)} mission(s) will run in dependency order.</p>
            <form phx-submit="approve">
              <label style="display:block; margin-bottom:0.5rem">
                <input type="radio" name="approve[mode]" value="existing" checked /> Existing sector
                <select name="approve[sector_id]" style="width:100%; margin-top:0.3rem; background:var(--ground); color:var(--ink); border:1px solid var(--line); border-radius:6px; padding:0.4rem">
              ...
              <label style="display:block; margin-bottom:0.9rem">
                ...
                <input type="text" name="approve[new_name]" placeholder="repo-name" style="width:100%; margin-top:0.3rem; background:var(--ground); color:var(--ink); border:1px solid var(--line); border-radius:6px; padding:0.4rem" />
              </label>
  ...
  defp brief_board(assigns) do
    ...
    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:0.75rem">
      <div class="card" style="grid-column: 1 / -1; display:flex; gap:1rem">
        <div style="flex:1">
          <div style="font-size:0.7rem; color:var(--ink-3); text-transform:uppercase">Vision</div>
      ...
      <% else %>
        <svg viewBox="0 0 800 520" style="width:100%; max-height:520px">
          <%= for edge <- @edges do %>
            ...
            <text x={(edge.x1 + edge.x2) / 2} y={(edge.y1 + edge.y2) / 2 - 6} fill="var(--ink-3)" font-size="10" text-anchor="middle">{edge.kind}</text>
          <% end %>
          <%= for m <- @modules do %>
            <circle cx={m.x} cy={m.y} r={radius(m.effort)} fill="rgba(88,166,255,0.15)" stroke="var(--accent)" stroke-width="2" />
            <text x={m.x} y={m.y} fill="var(--ink)" font-size="13" text-anchor="middle" dominant-baseline="middle">{m.label}</text>
          <% end %>
        </svg>
      <% end %>
    </div>
      ...
      <% else %>
        <%= for {level, idx} <- Enum.with_index(@levels) do %>
          <div style="display:flex; gap:0.75rem; margin-bottom:0.75rem; align-items:stretch">
            <div style="color:var(--ink-3); font-size:0.75rem; writing-mode:vertical-rl; text-align:center">wave {idx + 1}</div>
            <%= for item <- level do %>
              <div style="flex:1; border:1px solid var(--line); border-radius:8px; padding:0.6rem; background:var(--ground); max-width:320px">
                ...
                <div style="font-size:0.78rem; color:var(--ink-3); max-height:4.5em; overflow:hidden">{String.slice(item.goal, 0, 220)}</div>
          ...
          <%= for m <- @studio.mockups do %>
            <div class="card" style="padding:0.5rem">
              <div style="display:flex; justify-content:space-between; margin-bottom:0.4rem">
                <span style="font-size:0.75rem; color:var(--ink-3)">{m.target} — <span style="color:var(--warn)">{m.style}</span></span>
            ...
            <div style="display:flex; gap:0.6rem; overflow-x:auto; padding-bottom:0.5rem">
              <%= for {panel, idx} <- Enum.with_index(board.panels, 1) do %>
                <div style="min-width:190px; max-width:190px; border:1px solid var(--line); border-radius:8px; background:var(--ground); padding:0.5rem">
                  ...
                  <div style="font-size:0.75rem; color:var(--ink-3)">{panel["description"]}</div>
                </div>
              <% end %>
            </div>
          </div>
  ...
  defp em(text),
    do: Phoenix.HTML.raw("<span style=\"color:var(--ink-3); font-style:italic\">#{text}</span>")
