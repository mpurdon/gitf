defmodule GiTF.Dashboard.DesignLive do
  ...
  def render(assigns) do
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem">
      <div>
        ...
        <div style="color:var(--ink-3); font-size:0.85rem">{@mission[:goal]}</div>
      </div>
    ...
    <div :if={@mode == :approval} class="panel" style="margin-top:1rem; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:0.75rem">
      <div style="display:flex; align-items:center; gap:0.5rem">
        <span style="color:var(--ink-3); font-size:0.85rem">Override selection:</span>
        <button
          :for={s <- @strategy_list}
  ...
  defp render_compare(assigns) do
    ...
    <div style="display:grid; grid-template-columns:repeat(3, 1fr); gap:1rem">
      <div :for={strategy <- @strategy_list} class={"strategy-card #{if winner?(strategy, @review), do: "selected"}"}>
        ...
        <div :if={is_nil(design)} style="text-align:center; padding:2rem 0; color:var(--ink-3)">
  ...
  defp render_decision_brief(assigns) do
    ...
    <div class="panel" style="margin-bottom:1rem">
      ...
      <div :if={is_nil(@report) && !@report_generating} style="color:var(--ink-3); font-size:0.85rem; margin-top:0.5rem">
      ...
      <div :if={@report}>
        ...
        <div :if={@report["convergence"]} style="font-size:0.9rem; color:var(--ink-2); margin-bottom:0.9rem">
          {@report["convergence"]}
        </div>
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(15rem, 1fr)); gap:1rem">
          <div :for={d <- get_list(@report, "designs")}>
            ...
            <div :for={n <- List.wrap(d["notable"])} style="font-size:0.82rem; color:var(--ink-3)">
              <span class="coverage-ok">✓</span> {n}
            </div>
            <div :for={m <- List.wrap(d["missed"])} style="font-size:0.82rem; color:var(--ink-3)">
        ...
        <div :if={get_list(@report, "watch_items") != []} style="margin-top:0.9rem">
          <div style="color:var(--ink-3); font-size:0.8rem; margin-bottom:0.35rem">Watch Items</div>
          <div :for={w <- get_list(@report, "watch_items")} class="issue-item issue-medium">
            <div style="font-size:0.85rem">{w["concern"]}</div>
            <div :if={w["why_it_matters"]} style="font-size:0.8rem; color:var(--ink-3); margin-top:0.2rem">
              → {w["why_it_matters"]}
            </div>
          </div>
        </div>
  ...
  defp render_file_matrix(assigns) do
    ...
    <div :if={@total_files > 0} class="panel" style="margin-top:1rem">
      ...
      <div style="color:var(--ink-3); font-size:0.85rem; margin:0.5rem 0 0.75rem">
    ...
    <div class="panel" style="margin-top:1rem">
      ...
      <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(15rem, 1fr)); gap:1rem; margin-top:0.75rem">
        <div :for={strategy <- @strategy_list}>
          ...
          <div :if={design && get_list(design, "risks") == []} style="color:var(--ink-3); font-size:0.85rem">
            None recorded.
          </div>
          <div :if={is_nil(design)} style="color:var(--ink-3); font-size:0.85rem">Generating…</div>
        </div>
      </div>
    </div>
    """
    ...
    <div>
      <div>
        ...
        <div :if={is_nil(@design)} class="panel" style="text-align:center; padding:3rem">
          ...
          <div style="margin-top:0.75rem; color:var(--ink-3)">Design generation in progress...</div>
        </div>
      </div>
    ...
    <div class="panel" style="margin-top:1rem">
      ...
      <div :if={@review}>
        <div style="margin:0.75rem 0">
          <div style="color:var(--ink-3); font-size:0.8rem; margin-bottom:0.25rem">Selected Design</div>
        ...
        <div class="review-split">
          <div style="margin:0.75rem 0">
            <div style="color:var(--ink-3); font-size:0.8rem; margin-bottom:0.35rem">Coverage</div>
            <div :for={cov <- get_list(@review, "coverage")} class="coverage-item">
              ...
              <span :if={@requirement_index[cov["req_id"]]} style="color:var(--ink-3)">
          ...
          <div style="margin:0.75rem 0">
            <div style="color:var(--ink-3); font-size:0.8rem; margin-bottom:0.35rem">Issues ({length(get_list(@review, "issues"))})</div>
            <div :for={issue <- sort_issues(get_list(@review, "issues"))} class={"issue-item issue-#{issue["severity"] || "low"}"}>
              ...
              <div :if={issue["suggestion"]} style="font-size:0.8rem; color:var(--ink-3); margin-top:0.2rem">→ {issue["suggestion"]}</div>
            </div>
        ...
        <div style="margin:0.75rem 0">
          <div style="color:var(--ink-3); font-size:0.8rem; margin-bottom:0.25rem">Risk Assessment</div>
          <div style="font-size:0.85rem">{@review["risk_assessment"] || "—"}</div>
        </div>
      </div>

      <div :if={is_nil(@review)} style="text-align:center; padding:2rem 0; color:var(--ink-3)">
  ...
  defp render_prompt_content(assigns) do
    ~H"""
    <div style="padding:0.5rem 0; color:var(--ink-3); font-size:0.85rem; white-space:pre-wrap">{@content}</div>
    """
  end
  ...
  defp render_components(assigns) do
    ...
    <div :for={comp <- get_list(@design, "components")} class="component-card">
      <div style="font-weight:600; color:var(--ink); margin-bottom:0.3rem">{comp["name"] || "unnamed"}</div>
      <div style="font-size:0.85rem; color:var(--ink-3); margin-bottom:0.4rem">{comp["description"]}</div>
      <div :if={comp["files"] && comp["files"] != []} style="margin-bottom:0.3rem">
        <span :for={f <- List.wrap(comp["files"])} class="file-tag">{f}</span>
      </div>
      <div :if={comp["interfaces"] && comp["interfaces"] != []} style="font-size:0.8rem; color:var(--ink-3)">
        <code :for={iface <- List.wrap(comp["interfaces"])} style="background:var(--line-soft); padding:0.1rem 0.4rem; border-radius:3px; margin-right:0.3rem">{iface}</code>
      </div>
    </div>
    """
  end
    ...
    <table style="width:100%; margin-top:0.25rem">
      ...
      <tbody>
        <tr :for={req <- get_list(@design, "requirement_mapping")}>
          ...
          <td>{req["component"]}</td>
          <td style="font-size:0.85rem; color:var(--ink-3)">{req["approach"]}</td>
        </tr>
      </tbody>
    </table>
    """
  ...
  defp render_dependencies(assigns) do
    ...
    <div style="padding:0.5rem 0">
      <div :for={dep <- get_list(@design, "dependencies")} style="font-size:0.85rem; padding:0.2rem 0">
        <code>{dep["from"]}</code>
        <span style="color:var(--ink-3); margin:0 0.5rem">→</span>
        <code>{dep["to"]}</code>
      </div>
      <div :if={get_list(@design, "dependencies") == []} style="color:var(--ink-3); font-size:0.85rem">No dependencies listed.</div>
    </div>
    """
  end
  ...
  defp render_risks(assigns) do
    ...
    <div style="padding:0.5rem 0">
      ...
      <div :if={get_list(@design, "risks") == []} style="color:var(--ink-3); font-size:0.85rem">No risks identified.</div>
    </div>
    """
  end
  ...
  defp strategy_color(strategy), do: Map.get(@strategy_colors, strategy, "var(--ink-3)")
end
