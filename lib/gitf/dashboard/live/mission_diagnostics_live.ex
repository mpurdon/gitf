              ...
              <div class={"timeline-item #{if idx == length(@transitions) - 1 && stuck?(@mission, t), do: "timeline-item-stuck"}"}>
                ...
                <div class="timeline-content">
                  ...
                  <%= if Map.get(t, :reason) do %>
                    <div style="margin-top:0.35rem; font-size:0.85rem; color:var(--ink-3)">
                      {Map.get(t, :reason)}
                    </div>
                  <% end %>
                </div>
      ...
      <%= if @failure_info do %>
        <div class="panel" style="margin-bottom:1rem; border-left:3px solid var(--crit)">
          ...
          <div style="display:flex; gap:1rem; flex-wrap:wrap; align-items:center">
            ...
            <span style="color:var(--ink-3)">during <strong>{@failure_info[:failure_phase] || "?"}</strong> phase</span>
            <%= if length(@failure_info[:failed_op_ids] || []) > 0 do %>
              <span style="color:var(--ink-3)">{length(@failure_info[:failed_op_ids])} failed ops</span>
            <% end %>
          </div>
          <div style="margin-top:0.5rem; font-size:0.85rem; color:var(--ink-2)">{@failure_info[:failure_reason]}</div>
        </div>
      <% end %>
          ...
          <%= for op <- @failed_ops, op.id == @selected_op do %>
            ...
            <div style="margin-bottom:1rem">
              <div style="display:flex; gap:0.5rem; align-items:center; flex-wrap:wrap; margin-bottom:0.5rem">
                <strong style="color:var(--ink)">{Map.get(op, :title, "Untitled Op")}</strong>
                ...
                <%= if Map.get(op, :ghost_id) do %>
                  <span style="font-size:0.8rem; color:var(--ink-3)">Ghost: {short_id(op.ghost_id)}</span>
                <% end %>
            ...
            <%= if Map.get(op, :error_message) do %>
              <div style="margin-bottom:1rem">
                <div style="font-size:0.85rem; font-weight:600; color:var(--ink); margin-bottom:0.35rem">Error</div>
            ...
            <%!-- Failure Analysis --%>
            <div style="margin-bottom:1rem">
              <div style="font-size:0.85rem; font-weight:600; color:var(--ink); margin-bottom:0.35rem">Failure Analysis</div>
              <%= if analysis = Map.get(@analyses, op.id) do %>
                <div style="display:flex; gap:0.5rem; align-items:center; margin-bottom:0.5rem; flex-wrap:wrap">
                  ...
                  <%= if count = Map.get(analysis, :similar_count) do %>
                    <span style="font-size:0.8rem; color:var(--ink-3)">{count} similar failures</span>
                  <% end %>
                </div>
                <%= if Map.get(analysis, :root_cause) do %>
                  <div style="font-size:0.85rem; color:var(--ink-2); margin-bottom:0.5rem">{analysis.root_cause}</div>
                <% end %>
                <%= if suggestions = Map.get(analysis, :suggestions) do %>
                  <ul style="margin:0; padding-left:1.25rem; font-size:0.85rem; color:var(--ink-3)">
              ...
              <% else %>
                <%= if MapSet.member?(@analyzing, op.id) do %>
                  <div style="display:flex; align-items:center; gap:0.5rem; color:var(--ink-3)">
            ...
            <%= if chain = find_retry_chain(@retry_chains, op.id) do %>
              <div style="margin-bottom:1rem">
                <div style="font-size:0.85rem; font-weight:600; color:var(--ink); margin-bottom:0.35rem">Retry Chain</div>
            ...
            <%= if ghost_data = Map.get(@ghost_info, op.id) do %>
              <div style="margin-bottom:1rem">
                <div style="font-size:0.85rem; font-weight:600; color:var(--ink); margin-bottom:0.35rem">Ghost Context</div>
            ...
            <%= if audit = Map.get(@audit_data, op.id) do %>
              <div style="margin-bottom:1rem">
                <div style="font-size:0.85rem; font-weight:600; color:var(--ink); margin-bottom:0.35rem">Audit Results</div>
                <div class="grid-2">
                  <%= for {label, key} <- [{"Quality", :quality_score}, {"Static", :static_score}, {"Security", :security_score}] do %>
                    <%= if score = Map.get(audit, key) do %>
                      <div>
                        <div style="font-size:0.8rem; color:var(--ink-3); margin-bottom:0.25rem">{label}: {score}/100</div>
                        <div class="score-bar">
                ...
                <%= if Map.get(audit, :issue_count) do %>
                  <div style="margin-top:0.5rem; font-size:0.85rem; color:var(--ink-3)">
            ...
            <div style="border-top:1px solid var(--line); padding-top:1rem; margin-top:0.5rem">
              <div style="font-size:0.85rem; font-weight:600; color:var(--ink); margin-bottom:0.5rem">Recovery Actions</div>
              <div style="display:flex; gap:0.5rem; flex-wrap:wrap; align-items:flex-start">
                <%!-- Smart Retry --%>
                <%= if strategy = Map.get(@suggested_strategies, op.id) do %>
                  <div style="display:flex; flex-direction:column; gap:0.35rem">
                    <span style="font-size:0.8rem; color:var(--ink-3)">Strategy: <strong style="color:var(--warn)">{strategy_label(strategy)}</strong></span>
        ...
        <div class="grid-2">
          <div>
            <%= for {component, status} <- Map.get(@health, :checks, %{}) do %>
              <div style="display:flex; justify-content:space-between; padding:0.35rem 0; border-bottom:1px solid var(--line-soft); font-size:0.85rem">
                <span>{component}</span>
            ...
            <%= if @alerts != [] do %>
              <%= for {rule, message} <- @alerts do %>
                <div style="background:var(--warn)15; border:1px solid var(--warn)33; border-radius:6px; padding:0.5rem 0.75rem; margin-bottom:0.5rem; font-size:0.85rem">
                  ...
                  <div style="color:var(--ink-3); margin-top:0.15rem">{message}</div>
                </div>
              <% end %>
            <% else %>
