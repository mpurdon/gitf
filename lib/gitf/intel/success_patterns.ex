defmodule GiTF.Intel.SuccessPatterns do
  ...
  def get_best_practices(sector_id) do
    ...
    if Enum.empty?(patterns) do
      ...
      %{
        common_factors:
          Enum.map(factor_frequency, fn {factor, count} ->
  ...
  defp identify_success_factors(op) do
    ...
    # Audit passed
    factors =
      if Map.get(op, :verification_status) == "passed" do
    ...
    # No retries
    factors =
      if is_nil(Map.get(op, :retry_of)) do
  ...
  defp generate_suggestions(best_practices) do
    ...
    # Model suggestion
    suggestions =
      if best_practices.recommended_model do
    ...
    # Quality expectation
    suggestions =
      if best_practices.average_quality do
