defmodule GiTF.Schema.Op do
  ...
  @type t :: %__MODULE__{
          ...
          provider_retry_count: non_neg_integer(),
          failure_classification: atom() | nil,
          failure_judgement: map() | nil,
    ...
    if missing != [] do
      ...
      {:ok,
       %{
         ...
         strategy: attrs[:strategy] || attrs["strategy"],
         ...
         provider_retry_count: attrs[:provider_retry_count] || attrs["provider_retry_count"] || 0,
         failure_classification:
           attrs[:failure_classification] || attrs["failure_classification"],
         failure_judgement: attrs[:failure_judgement] || attrs["failure_judgement"],
         last_failure_reason: attrs[:last_failure_reason] || attrs["last_failure_reason"],
  ...
  def from_map(raw) when is_map(raw) do
    %__MODULE__{
      ...
      provider_retry_count: raw[:provider_retry_count] || raw["provider_retry_count"] || 0,
      failure_classification: raw[:failure_classification] || raw["failure_classification"],
      failure_judgement: raw[:failure_judgement] || raw["failure_judgement"],
      last_failure_reason: raw[:last_failure_reason] || raw["last_failure_reason"],
