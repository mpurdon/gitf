defmodule GiTF.Intel do
  ...
  def get_prompt_context(sector_id, phase, mission) when is_map(mission) do
    ...
    [historical, knowledge, operator_context(mission), invitation]
    |> Enum.reject(&(&1 in [nil, ""]))
    |> Enum.join("\n\n")
  end

  @doc """
  What the operator has said about this work: answered questions and, on
  a resumed child, the approval rejections that sent an earlier run back.
  Every prompt that carries one must carry both — a fix ghost that saw
  the decisions but not the rejection re-validated blind to the one
  thing the operator said was wrong. Best-effort: an unreadable store
  costs a block, never a prompt.
  """
  @spec operator_context(map()) :: String.t()
  def operator_context(mission) when is_map(mission) do
    [operator_decisions(mission), GiTF.Major.PhasePrompts.render_rejection_notes_block(mission)]
    |> Enum.reject(&(&1 == ""))
    |> Enum.join("\n\n")
  end
